...
test('opacity slider drives overlay opacity', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    const slider = document.getElementById('heli-opacity');
...
test('opacity reset restores default 0.6', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    slider.value = '0.3';
    slider.dispatchEvent(new Event('input'));
    document.getElementById('heli-opacity-reset').click();
    const img = document.querySelector('.leaflet-overlay-pane img.leaflet-image-layer');
    return {
      sliderVal: slider.value,
      opacity: parseFloat(img.style.opacity),
      label: document.getElementById('heli-opacity-val').textContent,
    };
  });
...
test('toggle state and opacity persist across reload', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const s = document.getElementById('heli-opacity');
    s.value = '0.5';
  ...
  await expect(page.locator('#heli-controls')).toBeVisible();
  expect(await page.evaluate(() => document.getElementById('heli-opacity').value)).toBe('0.5');
  await expect(page.locator('.leaflet-overlay-pane img.leaflet-image-layer').first()).toBeVisible();
});
