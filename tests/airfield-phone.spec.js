// @ts-check
// The airfield panel offers the field's phone as a link that dials it.
//
// The call-sign catalog has carried a phone number for most fields all along; only the
// parking-request form used it, as text to copy by hand. A pilot who needs the field on the
...
test('a field with a published number shows it as a tel: link', async ({ page }) => {
  await openField(page, 'LLHZ');                      // Herzliya: 09-9719554 in the catalog
  const link = page.locator('#inspector .airfield-phone');
...
test('the frequencies stay together, with the phone after them', async ({ page }) => {
  ...
  const order = await page.evaluate(() => [...document.querySelectorAll('#inspector .comm-section .row')]
    .map(r => r.classList.contains('airfield-phone-row') ? 'phone' : 'freq'));
...
test('a field with no call sign gets no phone row, not an empty one', async ({ page }) => {
  ...
  await expect(page.locator('#inspector .airfield-phone-row')).toHaveCount(0);
});
...
test('in Hebrew the number still reads left to right', async ({ page }) => {
  ...
  await expect(page.locator('#inspector .airfield-phone-row label')).toHaveText('טלפון');
...
test('the label says Phone, not Tower phone -- most of these fields have no tower', async ({ page }) => {
  await openField(page, 'LLHZ');
  await expect(page.locator('#inspector .airfield-phone-row label')).toHaveText('Phone');
});
