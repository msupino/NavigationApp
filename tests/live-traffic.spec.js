// @ts-check
// ADS-B traffic on the moving map, while a fix is driving it.
//
...
async function boot(page, body = { aircraft: AC }) {
...
test('nothing is drawn, and nothing is fetched, on a planning map', async ({ page }) => {
  let asked = 0;
  await page.route(/api\/traffic.*/, r => { asked++; r.fulfill({ status: 200, contentType: 'application/json', body: '{"aircraft":[]}' }); });
  await page.addInitScript(() => { navigator.geolocation.watchPosition = () => 1; });
  await page.goto('?lang=en&nogist');
  await page.waitForFunction(() => typeof window.trafficRefresh === 'function');
  await page.evaluate(() => window.trafficRefresh());
  await page.waitForTimeout(500);
  expect(await marks(page).count()).toBe(0);
  expect(asked).toBe(0);            // no fix, no request: the feature costs nothing at a desk
});

test('a fix brings the traffic, and losing it takes it away', async ({ page }) => {
  await boot(page);
  await fly(page);
