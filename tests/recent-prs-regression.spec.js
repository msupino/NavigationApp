...
test.describe('#238 — toolbar section order', () => {
  test('sections render in order: build, view, display, charts, export, weather, print', async ({ page }) => {
    await boot(page);
    const order = await page.locator('#toolbar .tb-section').evaluateAll(els =>
      els.map(el => el.getAttribute('data-sec')));
    expect(order).toEqual(['build', 'view', 'display', 'charts', 'export', 'weather', 'print']);
  });
});
