...
- **State:**
  - `state.waypoints[i]` = `{lat, lng, name, hotspot?}` (name optional).
    ...
    and share links. The Route info `Show hotspots` checkbox controls only graph-derived
    defaults without changing explicit waypoint choices: an explicit `hotspot: true` stays
    visible and an explicit `hotspot: false` stays hidden. Its `defaultShowHotspots` tune
    controls the initial value and defaults to on. The selected outbound/return direction also
...
- Airfield inspectors keep their NOTAM list scoped to the selected ICAO. The
  ...
  entry. Clicking an airfield's NOTAM count badge on the map opens the regular
  NOTAM sheet with that ICAO preselected in its filter, so the pilot keeps both
  the future-entry checkbox and access to the full feed. The choice is transient
  and adds no `localStorage` key.
